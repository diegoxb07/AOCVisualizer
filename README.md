...
- **Checks:** `node tests/run-tests.js` runs the parser checks (synthetic fixtures; a FLAG means a parser judgment call does not match its independently computed expectation). Verify UI changes by opening the page and exercising the load and playback flow.
