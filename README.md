...
> **TDR Mode:** when the archive has Tail Doppler Radar for the loaded mission, a **TDR Mode** button appears in the map header (it reads "TDR Loading…" until data is usable). Clicking it opens the radar workspace: the tracker pins over the page in a dedicated 3D radar view, playback jumps ahead to the first radar leg, and the camera parks overhead of the recorded coverage, with a sidebar listing the levels grouped into pressure layers (millibars, 900-1000 mb up through under 100 mb) to display alone or combined, plus leg buttons that show a chosen leg alone at its end. The radar scans in around the aircraft as it flies each leg (every leg preloads, so transitions are seamless), re-scanned areas show the newest data wherever the new pass recorded any, and a tall white beam marks the aircraft. **2D cross section mode** exits to the 2D map, centered on the storm with only upper layers drawn, to pick two points for a vertical slice, drawn like the measure tool; picked layers also draw over the 2D map. Closing the workspace restores the previous view exactly: the regular tracker never shows radar and its camera is never moved. Only post-season quality-controlled (level 2) radar is used; a mission carrying just the real-time product shows the TDR control as unavailable, and missions before 2021 use an older grid format the API cannot render.
