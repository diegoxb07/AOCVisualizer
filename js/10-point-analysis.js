/* Mission Visualizer - point analysis modal + report download
   ...
   Loaded as a classic (non-module) script; all parts share one global scope, in order. */
    ...
    function processPointAnalysisPlotting(dataRow) {
        ...
        let statsHTML = `
            <div style="color:#2dd4bf; font-weight:bold; font-size:15px; border-bottom:1px solid #20262f; padding-bottom:8px; margin-bottom:14px; font-family:monospace; margin-top:12px;">STATION REPORT [${dataRow.time.slice(0,2)}:${dataRow.time.slice(2,4)}:${dataRow.time.slice(4)} UTC]</div>
        ...
        if (targetPr !== 850 || availableMetrics.has('pressure') || availableMetrics.has('sfcPr')) { statsHTML += `<p>BARO LEVEL  : <span style="color:#5eead4; font-weight:bold;">${sf(targetPr, 1)} mb</span></p>`; }
        ...
        if (useGps) { if (availableMetrics.has('gpsAlt')) statsHTML += `<p>GPS ALTITUDE: <span style="color:#2dd4bf; font-weight:bold;">${gAltDisp}</span></p>`; } 
        ...
        if (availableMetrics.has('tempr') || availableMetrics.has('dewpt')) {
            ...
            statsHTML += `<p>ENVIRONMENT : <span style="color:#ef4444; font-weight:bold;">${tDisp}</span> / <span style="color:#34e3a2; font-weight:bold;">${tdDisp}</span></p>`;
        }
        
        if (rhCalc !== null) statsHTML += `<p>COMPUTED RH : <span style="color:#a78bfa; font-weight:bold;">${sf(rhCalc, 1)}%</span></p>`;
        ...
        if (availableMetrics.has('accZ')) statsHTML += `<p>VERT ACCEL  : <span style="color:#f0369e; font-weight:bold;">${sf(dataRow.accZ, 2)} m/s²</span></p>`;
        if (availableMetrics.has('pitch') || availableMetrics.has('roll')) statsHTML += `<p>PITCH / ROLL: <span style="color:#2dd4bf; font-weight:bold;">${sf(dataRow.pitch, 1)}° / ${sf(dataRow.roll, 1)}°</span></p>`;
        if (availableMetrics.has('driftAngle')) statsHTML += `<p>DRIFT ANGLE : <span style="color:#ff7ad9; font-weight:bold;">${sf(dataRow.driftAngle, 1)}°</span></p>`;
