/* Mission Visualizer - filter window init + video-clock sync
   ...
   Loaded as a classic (non-module) script; all parts share one global scope, in order. */
    ...
    function applyFiltersAndInit(shouldPlay = false) {
        isPlaying = false; playPauseBtn.innerHTML = PLAY_ICON + "Play"; if (animationFrameId) cancelAnimationFrame(animationFrameId);
        ...
        if (shouldPlay === true) {
            isPlaying = true; playPauseBtn.innerHTML = PAUSE_ICON + "Pause"; playbackAccumulator = 0; lastTickTime = performance.now(); 
